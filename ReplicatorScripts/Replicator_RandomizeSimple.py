...
class CowWriter(Writer):
    ...
    def _write_bounding_box_data(self, data: dict, bbox_type: str, render_product_path: str, annotator: str):
        ...
        count = 0
        print(bbox_data_all)
        labels_file_path = f"{render_product_path}rgb_{self._sequence_id}{self._frame_id:0}.txt"
        for bbox_data in bbox_data_all:
            ...
            semantic_label = data[annotator]["info"]["idToLabels"].get(bbox_data["semanticId"])
            coco_bbox_data = []

            coco_bbox_data.append(semantic_label)
            coco_bbox_data.append(str((float(target_bbox_data["x_min"]) + float(target_bbox_data["x_max"]))/2))
            coco_bbox_data.append(str((float(target_bbox_data["y_min"]) + float(target_bbox_data["y_max"]))/2))
            coco_bbox_data.append(str(width))
            coco_bbox_data.append(str(height))
            ...
            writer.writerows(target_coco_bbox_data)

            self._backend.write_blob(labels_file_path, bytes(buf.getvalue(), "utf-8"))

rep.WriterRegistry.register(CowWriter)

camera_positions = [(-1100, 1480, -900), (-1100, 3355, -900),
                    (2815, 3410, -800), (2815, 1380, -800)]
...
def randomize_pigsdirty():
    pigs = rep.get.prims(semantics=[('class', 'pig_dirty')])
    with pigs:
        rep.modify.visibility(rep.distribution.choice([True, False]))
    return pigs.node

rep.randomizer.register(randomize_pigsdirty)

def randomize_pigsclean():
    pigs = rep.get.prims(semantics=[('class', 'pig_clean')])
    with pigs:
        rep.modify.visibility(rep.distribution.choice([True, False]))
    return pigs.node

rep.randomizer.register(randomize_pigsclean)
...
with rep.trigger.on_frame(num_frames=3000):
    with camera:
        rep.modify.pose(position=rep.distribution.choice(
            camera_positions), look_at=(790, 2475, -1178))
    rep.randomizer.randomize_environment()
    rep.randomizer.randomize_pigsdirty()
    rep.randomizer.randomize_pigsclean()
    rep.randomizer.randomize_cows2()
    rep.randomizer.randomize_cows3()
    rep.randomizer.randomize_cows4()

writer = rep.WriterRegistry.get('BasicWriter')
writer.initialize(output_dir='C:/Users/anura/Desktop/IndoorRanch_ReplicatorOutputs/SanjRun1',
                  rgb=True,  bounding_box_2d_tight=True, bounding_box_2d_loose=True, semantic_segmentation=True, bounding_box_3d=True)
